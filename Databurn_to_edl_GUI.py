...
def process_video_thread(log_widget, progress):
    ...
    try:
        ...
        progress["maximum"] = len(os.listdir(output_folder))

        for idx, image_file in enumerate(sorted(os.listdir(output_folder))):
            if cancel_processing:
                log_message("Traitement annulé par l'utilisateur.", log_widget)
            ...
            if not timecode:
                log_message(f"[DEBUG] Aucun timecode trouvé pour l'image.", log_widget)
                timecode = "00:00:00:00"

            if not filename:
                log_message(f"[DEBUG] Aucun nom de fichier trouvé pour l'image.", log_widget)
